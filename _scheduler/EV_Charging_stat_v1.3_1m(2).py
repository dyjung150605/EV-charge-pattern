...
final_data = pd.DataFrame([stat_ids, chger_ids, busiids,
                          stat_upddts, lasttsdts, lasttedts, nowtsdts, states]).T
final_data.columns = ['충전소ID', '충전기ID', '기관아이디',
                      '상태갱신일시', '최종충전시작', '최종충전종료', '충전중시작', '실시간 상태(15분)']

root_path = os.getcwd()
# 기본 폴더에 저장한다.
final_data.to_csv(root_path + "/output_STAT_1m/" + nowtext[:-2] + "00" + ".csv",
                  mode='w', encoding='utf-8-sig')

# 하루짜리 폴더에 저장을 시도한다.
sub_path = os.getcwd() + "/output_STAT_1m/" + nowtext[:-6] + "/"
try:
    ...
    final_data.to_csv(sub_path + nowtext[:-2] + "00" + ".csv",
                      mode='w', encoding='utf-8-sig')
