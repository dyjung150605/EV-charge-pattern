...
for z in code_region:
    ...
    while 1:
        ...
        # request 오류 발생에 대한 해결
        # 출처: https://hatpub.tistory.com/110 [잘먹는 인디 개발자:티스토리]
        while(True):
...
final_data = pd.DataFrame([names, stat_ids, chger_ids, charge_types, addresss, latitudes, longitudes,
                           stat_upddts, lasttsdts, lasttedts, nowtsdts, outputs, methods, zcodes, limityns, limitdetails, parking_frees, states]).T
final_data.columns = ['충전소명', '충전소ID', '충전기ID', '충전타입', '주소', '위도', '경도', '상태갱신일시', '최종충전시작', '최종충전종료', '충전중시작',
                      '출력', '충전방식', '지역코드', '이용자제한', '이용자제한사유', '무료 주차', '실시간 상태(15분)']

save_path = "D:/_scheduler/output/"
final_data.to_csv(save_path + nowtext[:-2] + "00" + ".csv",
